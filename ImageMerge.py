...
def start():
    ...
    # 파일 목록 확인
    if list_file.size() == 0:
        msgbox,msgbox.showwarning("경고", "이미지 파일을 추가하세요")
        return
...
# 저장경로 프레임
path_frame = LabelFrame(win, text="저장경로")
...
# 옵션 프레임
frame_option = LabelFrame(win, text="옵션")
...
# 진행 상황 Progress Bar
frame_progress = LabelFrame(win, text="진행상황")
...
btn_close = Button(frame_run, padx=5, pady=5, text="닫기", width=12, command=win.quit)
